...
- Preserve UTF-8 output, prefix it with a BOM for spreadsheet encoding detection, and keep `incidentes-YYYYMMDD.csv` naming.
...
1. Define typed incident filter criteria and extract the environment-agnostic filtering, CSV serialization, and filename formatting logic into `src/utils/incidents.ts`.
...
6. Add deterministic tests beside the utility in `src/utils/incidents.test.ts`.
...
- The output starts with UTF-8 BOM bytes `EF BB BF`; quotes, commas, CR/LF, accented characters, and empty values serialize correctly.
...
- [ ] CSV escaping, UTF-8 BOM output, and accented-character rendering meet the stated rules.
